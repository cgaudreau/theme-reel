...
module.exports = {
  ...
  chooseTheme: function (offset) {
    var themeNames = atom.themes.getLoadedNames().filter(function (name) {
      return name.substring(name.length - 3) !== '-ui';
    });

    var currentTheme = atom.themes.getActiveNames()[0];
    ...
    var uiTheme = atom.themes.getActiveNames()[1] || atom.themes.getActiveNames()[0];

    atom.themes.setEnabledThemes([uiTheme, newTheme]);
